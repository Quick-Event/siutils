	...
	: Super(parent)
{
	...
	connect(m_rxTimer, &QTimer::timeout, this, [this]() {
		nError() << this << "SiCommand timeout after" << (m_rxTimer->interval() / 1000.) << "sec.";
...
void SiTaskSetDirectRemoteMode::onSiMessageReceived(const SIMessageData &msg)
{
	...
	if(cmd == SIMessageData::Command::SetDirectRemoteMode) {
		...
		nError() << "Invalid command:" << (int)cmd << "received";
	}
...
void SiTaskReadStationBackupMemory::onSiMessageReceived(const SIMessageData &msg)
{
	if(m_state == State::SwitchToRemote) {
		...
		if(cmd == SIMessageData::Command::GetSystemData) {
			...
			m_memoryDataPointer = (m_memoryDataPointer << 8) +  (uint8_t)hdr[ix++];
			m_memoryDataPointer = (m_memoryDataPointer << 8) +  (uint8_t)hdr[ix++];
			logCardRead() << "backup memory pointer:" << "0x" + QString::number(m_memoryDataPointer, 16).toStdString();
		...
		if(cmd == SIMessageData::Command::GetSystemData) {
			...
			if(m_blockCount > 255) {
				nError() << "Invalid block count:" << m_blockCount;
				abort();
				return;
			}
		...
		if(cmd == 0x81) {
			...
			logCardRead() << "read data pointer:" << QString::number(m_readDataPointer, 16).toStdString()
						  << "memory data pointer:" << QString::number(m_memoryDataPointer, 16).toStdString()
						  << "bytes to load:" << QString::number(m_memoryDataPointer - m_readDataPointer, 16).toStdString();
			...
			nError() << "Invalid command:" << cmd << "received";
			abort();
			return;
		}
	}
...
{
	if(ok) {
		logCardRead() << m_card.toString().toStdString();
		if(!m_withAutosend)
			sendACK();
	}
...
void SiTaskReadCard5::onSiMessageReceived(const SIMessageData &msg)
{
	...
	if(cmd == SIMessageData::Command::GetSICard5) {
		...
		int base = 5;
		nDebug() << "Card5 data";
		nDebug() << SIMessageData::dumpData(data.mid(base), 16).toStdString();
		...
		nError() << "Invalid command:" << "0x" + QString::number((int)cmd, 16).toStdString() << "received";
		abort();
	}
}
...
void SiTaskReadCard6::onSiMessageReceived(const SIMessageData &msg)
{
	...
	if(cmd == SIMessageData::Command::GetSICard6) {
		...
		nDebug() << SIMessageData::dumpData(data.mid(base), 4).toStdString();
		if(block_number == 0) {
			...
			if(m_cardSerie == Card6 || m_cardSerie == Card6Star) {
				...
				nError() << "block:" << block_number << "unsupported card serie:" << m_cardSerie << cardSerieToString(m_cardSerie);
				abort();
			}
		}
		else {
			if(block_number >= 1 && block_number <= 7) {
				if(m_card.cardNumber() > 0) {
					...
					nError() << "Card6 unexpected block number:" << block_number
							  << ", block #0 must be read first. See https://github.com/Quick-Event/quickbox/issues/732";
					if(m_withAutosend)
						nError() << "Try to switch auto-send mode off.";
					abort();
				}
			}
			else {
...
void SiTaskReadCard8::onSiMessageReceived(const SIMessageData &msg)
{
	...
	if(cmd == SIMessageData::Command::GetSICard8) {
		...
		if(block_number == 0) {
			...
			if(m_cardSerie == Card8) {
				if(block_number == 1) {
					...
					nError() << "Card8 unexpected block number:" << block_number;
					abort();
				}
			}
				...
				if(block_number == 3) {
					...
					logCardRead().nospace() << "MVBAT: " << mvbat << " 0x" << QString::number(mvbat, 16).toStdString();
					logCardRead().nospace() << "RBAT : " << rbat << " 0x" << QString::number(rbat, 16).toStdString();
					logCardRead().nospace() << "LBAT : " << lbat << " 0x" << QString::number(lbat, 16).toStdString() << " " << (lbat == 0xAA? "OK": "LOW");
					...
					nError() << "Siac unexpected block number:" << block_number;
					abort();
				}
			}
			else {
		...
		if(cmd == SIMessageData::Command::SICardRemoved)
			nError() << "seems like SI card was removed in middle of read-out process";
		abort();
	}
}
