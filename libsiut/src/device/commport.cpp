...
bool CommPort::openComm(const QString &_device, int _baudrate, int _data_bits, const QString &_parity_str, bool two_stop_bits)
{
	...
	QString device = _device;
	{
		...
		for(auto port : port_list) {
			...
			nDebug() << "\t" << port.portName().toStdString();
		}
...
void CommPort::emitCommInfo ( NecroLog::Level level, const QString& msg )
{
	//qfLog(level) << msg;
	switch (level) {
	case NecroLog::Level::Debug: nDebug() << msg.toStdString(); break;
	case NecroLog::Level::Info: nInfo() << msg.toStdString(); break;
	case NecroLog::Level::Warning: nWarning() << msg.toStdString(); break;
	default: nError() << msg.toStdString(); break;
	}
